...
def mass_to_gain(mass, pull):
    ...
    avg_mass_left = ((np.roll(mass, 1, axis=1) + mass) / 2).real
    avg_mass_right = ((np.roll(mass, -1, axis=1) + mass) / 2).real
    avg_mass_up = ((np.roll(mass, 1, axis=0) + mass) / 2).real
    avg_mass_down = ((np.roll(mass, -1, axis=0) + mass) / 2).real

    from_left = ((np.roll(pull, 1, axis=1) + pull) / 2).real * np.maximum(0, avg_mass_left)
    to_right = ((np.roll(pull, -1, axis=1) + pull) / 2).real * np.maximum(0, avg_mass_right)
    from_down = ((np.roll(pull, 1, axis=0) + pull) / 2).imag * np.maximum(0, avg_mass_up)
    to_up = ((np.roll(pull, -1, axis=0) + pull) / 2).imag * np.maximum(0, avg_mass_down)
